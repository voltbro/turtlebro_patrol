...
class EmergencyReaction(object):
    ...
    def __init__(self):
        ...
        self.curent_goal = 0
        self.home = [0, 0, 0, 0]  # position where to go if "home" command recieved
        self.stop_cmd = Twist()
    ...
    def patrol_control_command(self, alert):
        if alert.data in ("next", "shutdown", "pause", "resume", "home"): #to make sure command recieved is in list of commands
    ...
    def goal_message_assemble(self, target):
        ...
        goal.target_pose.pose.orientation = quaternion_from_euler(0, 0, float(target[3])) # using TF.transformation func()
    ...
    def goal_send(self, goal_to_send):
        ...
        self.log_pub.publish("Goal {} reached!".format(self.curent_goal))
        rospy.loginfo("Goal %s reached!" %self.curent_goal)

        #TODO verify _result == "OK or smthing" if goal preempted not sending controller("goal reached")
        self.controller("goal reached")
    ...
    def controller(self, cmd):
        ...
        if cmd == 'shutdown':
            ...
            self.client.cancel_goal()
            self.cmd_pub.publish(self.stop_cmd)
            self.goal_send(self.goal_message_assemble(self.home))
        elif cmd == 'goal reached':
            self.curent_goal += 1
            self.controller("next")
        else:
            if self.go_home == True:
                rospy.loginfo("Home reached waiting for command")
                self.log_pub.publish("Home reached waiting for command")
